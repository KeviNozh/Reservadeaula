...
def obtener_espacio_api(request, espacio_id):
    ...
    try:
        ...
        return JsonResponse({'success': False, 'error': f'Error al cargar espacio: {str(e)}'})

# === CREAR ESPACIO ===
@login_required
@csrf_exempt
def crear_espacio_api(request):
    """API para crear espacios"""
    ...
    try:
        ...
        if not data.get('tipo'):
            return JsonResponse({'success': False, 'error': 'El tipo es requerido'})
        
        if not data.get('capacidad'):
            return JsonResponse({'success': False, 'error': 'La capacidad es requerida'})
        
        # Procesar capacidad
        try:
            capacidad = int(data['capacidad'])
            if capacidad <= 0:
                return JsonResponse({'success': False, 'error': 'La capacidad debe ser mayor a 0'})
        except (ValueError, TypeError):
            return JsonResponse({'success': False, 'error': 'La capacidad debe ser un número válido'})
        
        # Procesar piso
        piso = None
        if data.get('piso') and str(data['piso']).strip():
            try:
                piso = int(data['piso'])
                if piso < 0:
                    return JsonResponse({'success': False, 'error': 'El piso no puede ser negativo'})
            except (ValueError, TypeError):
                return JsonResponse({'success': False, 'error': 'El piso debe ser un número válido'})
        
        # Crear espacio
        print("💾 Guardando espacio...")
        espacio = Espacio(
        ...
        print(f"✅ ESPACIO CREADO: {espacio.id} - {espacio.nombre}")
        ...
        print("❌ Perfil de usuario no encontrado")
...
def actualizar_espacio_api(request, espacio_id):
    ...
    print(f"🚀 INICIANDO actualizar_espacio_api para ID: {espacio_id}")
    ...
    try:
        ...
        # Obtener espacio
        espacio = Espacio.objects.get(id=espacio_id)
        print(f"📝 Editando espacio: {espacio.nombre}")
        ...
        if 'piso' in data:
            piso = data['piso']
            if piso and str(piso).strip():
                try:
                    espacio.piso = int(piso)
                    if espacio.piso < 0:
                        return JsonResponse({'success': False, 'error': 'El piso no puede ser negativo'})
                except (ValueError, TypeError):
                    return JsonResponse({'success': False, 'error': 'El piso debe ser un número válido'})
...
def eliminar_espacio_api(request, espacio_id):
    """API para eliminar espacios - COMPLETA Y FUNCIONAL"""
    if request.method != 'POST':
        return JsonResponse({'success': False, 'error': 'Método no permitido'})
    ...
    try:
        ...
        return JsonResponse({
            'success': True, 
            'message': f'Espacio "{nombre_espacio}" eliminado exitosamente'
        })
...
@login_required
@csrf_exempt
def test_crear_espacio_api(request):
    """API de prueba para crear espacio"""
    if request.method == 'POST':
        try:
            # Datos de prueba
            espacio = Espacio(
            ...
            espacio.save()
            return JsonResponse({'success': True, 'message': 'Espacio de prueba creado', 'id': espacio.id})
        except Exception as e:
            return JsonResponse({'success': False, 'error': str(e)})
    return JsonResponse({'success': False, 'error': 'Método no permitido'})
