    ...
    if request.method == 'POST':
        try:
            ...
            try:
                ...
                print(f"📢 Notificación de admin creada para nuevo usuario: {user.username}")
            except Exception as notif_error:
                print(f"⚠️ Error en notificación de admin: {notif_error}")
...
def notificar_accion_admin(tipo, titulo, mensaje, usuario_admin=None, usuario_relacionado=None, 
                         reserva=None, espacio=None, request=None):
    """
    Función centralizada para crear notificaciones de administrador
    """
    try:
        from .services import NotificacionAdminService
        ...
        print(f"❌ Error en notificar_accion_admin: {e}")
        return None

# === DECORADOR PARA AUTOMATIZAR NOTIFICACIONES ===

def notificar_accion_admin_decorador(tipo, titulo_template):
    """
    Decorador para automatizar notificaciones de admin en las vistas
    """
    def decorador(view_func):
        def wrapper(request, *args, **kwargs):
            ...
            try:
                # Solo notificar si fue exitoso (puedes ajustar esta lógica)
                if hasattr(response, 'data') and response.data.get('success', False):
                    # Construir mensaje basado en el tipo
                    mensaje = titulo_template
                    
                    # Personalizar según el tipo de acción
                    if tipo == 'usuario_registrado' and 'user' in kwargs:
                        mensaje = f"Usuario {kwargs['user'].username} registrado exitosamente"
                    elif tipo == 'reserva_creada' and 'reserva' in kwargs:
                        mensaje = f"Reserva {kwargs['reserva'].id} creada por {kwargs['reserva'].solicitante.username}"
                ...
                print(f"⚠️ Error en decorador de notificación: {e}")
                # No fallar la vista principal por error en notificación
