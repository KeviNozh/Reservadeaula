...
def crear_espacio_api(request):
    ...
    try:
        ...
        if not data.get('nombre') or not data.get('tipo') or not data.get('capacidad'):
            return JsonResponse({'success': False, 'error': 'Nombre, tipo y capacidad son requeridos'})
        
        # Crear espacio directamente
