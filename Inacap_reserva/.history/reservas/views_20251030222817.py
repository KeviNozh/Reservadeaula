...
def admin_dashboard_view(request):
    """Dashboard para administradores - VERSIÓN MEJORADA CON DATOS REALES"""
    try:
        perfil = PerfilUsuario.objects.get(user=request.user)
        # ACTUALIZADO: Roles que pueden acceder al admin
        if perfil.rol not in ['Administrativo', 'Investigacion', 'Aprobador']:
            return redirect('dashboard')
        
        # Estadísticas REALES para admin
        ...
        total_estados = reservas_aprobadas + reservas_pendientes + reservas_rechazadas
        porcentaje_aprobadas = round((reservas_aprobadas / total_estados * 100) if total_estados > 0 else 0)
        porcentaje_pendientes = round((reservas_pendientes / total_estados * 100) if total_estados > 0 else 0)
        porcentaje_rechazadas = round((reservas_rechazadas / total_estados * 100) if total_estados > 0 else 0)
        
        # Ocupación por espacios (ejemplo simplificado)
        espacios_ocupacion = []
        for espacio in Espacio.objects.all()[:5]:
            reservas_espacio = Reserva.objects.filter(espacio=espacio, estado='Aprobada').count()
            ocupacion = min(100, reservas_espacio * 10)  # Ejemplo simplificado
        ...
        if reservas_pendientes > 0:
            alertas.append({
                ...
                'mensaje': f'Hay {reservas_pendientes} solicitudes esperando aprobación'
            })
        
        # Actividad reciente (últimas 5 reservas)
        actividad_reciente = []
        reservas_recientes = Reserva.objects.select_related('solicitante').order_by('-fecha_solicitud')[:5]
        for reserva in reservas_recientes:
            actividad_reciente.append({
                'usuario': reserva.solicitante.first_name or reserva.solicitante.username,
                'descripcion': f'Reservó {reserva.espacio.nombre} para {reserva.fecha_reserva}',
                'fecha': reserva.fecha_solicitud.strftime('%H:%M')
            })
        ...
        context = {
            ...
            'ocupacion_promedio': 65  # Ejemplo, puedes calcularlo
        }
        return render(request, 'reservas/dashboard_admin.html', context)
    except PerfilUsuario.DoesNotExist:
        return redirect('login')
