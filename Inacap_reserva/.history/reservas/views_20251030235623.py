...
def obtener_espacio_api(request, espacio_id):
    """API para obtener datos de un espacio específico - COMPLETA"""
    try:
...
def crear_espacio_api(request):
    """API para crear espacios - CORREGIDA"""
    if request.method != 'POST':
        return JsonResponse({'success': False, 'error': 'Método no permitido'})
...
def actualizar_espacio_api(request, espacio_id):
    """API para actualizar espacios - COMPLETA Y FUNCIONAL"""
    if request.method != 'POST':
        return JsonResponse({'success': False, 'error': 'Método no permitido'})
    ...
    try:
        ...
        if 'piso' in data:
            espacio.piso = int(data['piso']) if data['piso'] else None
