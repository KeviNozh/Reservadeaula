...
def login_view(request):
    ...
    if request.method == 'POST':
        try:
            ...
            # Si el usuario no existe y es usuario normal, registrarlo automáticamente
            if user is None and user_type == 'usuario' and not email.endswith('@inacapmail.com'):
                print(f"👤 Usuario no existe, registrando automáticamente: {email}")
                user = registrar_usuario_automatico(email, password, 'Usuario')
                if user:
                    # Autenticar al nuevo usuario registrado
                    user = authenticate(request, username=email, password=password)
