...
def crear_espacio_api(request):
    """API PARA CREAR ESPACIO - VERSIÓN SUPER SIMPLIFICADA"""
    print("=" * 60)
    print("🚀 INICIANDO crear_espacio_api - VERSIÓN SIMPLIFICADA")
    ...
    try:
        ...
        if not data.get('tipo'):
            return JsonResponse({'success': False, 'error': 'Tipo requerido'})
        if not data.get('capacidad'):
            return JsonResponse({'success': False, 'error': 'Capacidad requerida'})
        
        # Crear espacio con solo los campos esenciales
        espacio = Espacio(
            nombre=data['nombre'],
            tipo=data['tipo'],
            capacidad=int(data['capacidad']),
            estado='Disponible'  # Valor fijo para simplificar
        )
        
        print("💾 Guardando espacio...")
        espacio.save()
        print(f"✅ ÉXITO - Espacio guardado: {espacio.id}")
        ...
        print("❌ Error JSON:", e)
        return JsonResponse({'success': False, 'error': f'Error en JSON: {str(e)}'})
    except ValueError as e:
        print("❌ Error de valor:", e)
        return JsonResponse({'success': False, 'error': f'Error en datos: {str(e)}'})
    except Exception as e:
