...
def crear_espacio_api(request):
    """API para crear un nuevo espacio - VERSIÓN CON DEBUG"""
    try:
        ...
        if not data.get('tipo'):
            return JsonResponse({'success': False, 'error': 'El tipo es requerido'}, status=400)
        if not data.get('capacidad'):
            return JsonResponse({'success': False, 'error': 'La capacidad es requerida'}, status=400)
        
        # Validar que la capacidad sea un número positivo
        try:
            capacidad = int(data['capacidad'])
            if capacidad <= 0:
                return JsonResponse({'success': False, 'error': 'La capacidad debe ser un número positivo'}, status=400)
        except (ValueError, TypeError):
            return JsonResponse({'success': False, 'error': 'La capacidad debe ser un número válido'}, status=400)
        ...
        print(f"❌ Error JSON: {e}")
        return JsonResponse({'success': False, 'error': 'Error en los datos JSON'}, status=400)
    except Exception as e:
