...
def editar_espacio_view(request):
    """Vista para editar espacio (template) - VERSIÓN DEBUG"""
    try:
        perfil = PerfilUsuario.objects.get(user=request.user)
        if perfil.rol not in ['Administrativo', 'Investigacion', 'Aprobador']:
            print("❌ Usuario sin permisos para editar espacios")
            return redirect('dashboard')
        
        espacio_id = request.GET.get('id')
        print(f"🔍 Editando espacio ID: {espacio_id}")
        
        if not espacio_id:
            print("❌ No se proporcionó ID de espacio")
        ...
        print(f"❌ Espacio {espacio_id} no existe")
        ...
        print(f"💥 Error inesperado en editar_espacio_view: {str(e)}")
        import traceback
        traceback.print_exc()
        messages.error(request, 'Error al cargar el espacio')
        return redirect('gestion_espacios')
...
def actualizar_espacio_api(request, espacio_id):
    """API para actualizar espacios - VERSIÓN DEBUG"""
