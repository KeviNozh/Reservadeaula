...
def crear_espacio_api(request):
    """API para crear un nuevo espacio - VERSIÓN CORREGIDA"""
    try:
        # Verificar permisos de administrador
        perfil = PerfilUsuario.objects.get(user=request.user)
        if perfil.rol not in ['Administrativo', 'Investigacion', 'Aprobador']:
        ...
        if not data.get('nombre') or not data.get('tipo') or not data.get('capacidad'):
            return JsonResponse({'success': False, 'error': 'Nombre, tipo y capacidad son requeridos'}, status=400)
        ...
        print(f"❌ Error creando espacio: {str(e)}")
