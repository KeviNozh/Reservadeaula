        ...
        try:
            ...
            if user is not None and user.is_active:
                try:
                    ...
                    # 🔑 VERIFICACIÓN DE ACCESO SEGÚN TIPO DE LOGIN
                    is_admin_login = user_type == 'administrador'
                    is_user_login = user_type == 'usuario'
                    
                    # Roles que pueden acceder como administradores
                    admin_roles = ['Administrador', 'Aprobador', 'Investigador']
                    # Roles que pueden acceder como usuarios normales
                    user_roles = ['Usuario', 'Docente', 'Investigador']
                    
                    if is_admin_login and perfil.rol not in admin_roles:
                        return JsonResponse({
                            'success': False,
                            'message': f'No tienes permisos de administrador. Tu rol es: {perfil.rol}'
                        }, status=403)
                    
                    if is_user_login and perfil.rol not in user_roles:
                        return JsonResponse({
                            'success': False,
                            'message': f'Rol no autorizado para acceso de usuario. Tu rol es: {perfil.rol}'
                        }, status=403)
...
def reserva_exitosa(request):
    ...
    try:
        ...
        # CAMBIA ESTA LÍNEA - usa guión bajo en lugar de guión medio
