        ...
        try:
            ...
            if user is not None and user.is_active:
                try:
                    ...
                    # 🔑 VERIFICACIÓN CORREGIDA DE ACCESO
                    is_admin_login = user_type == 'administrador'
                    
                    # Roles que pueden acceder como administradores
                    admin_roles = ['Administrador', 'Aprobador']
                    # Roles que pueden acceder como usuarios normales
                    user_roles = ['Usuario', 'Docente', 'Investigador', 'Administrador', 'Aprobador']
                    ...
                    if is_admin_login:
                        ...
                        if perfil.rol not in admin_roles:
                            return JsonResponse({
                                'success': False,
                                'message': f'No tienes permisos de administrador. Tu rol es: {perfil.rol}. Solo Administrador y Aprobador pueden acceder aquí.'
                            }, status=403)
                    else:
