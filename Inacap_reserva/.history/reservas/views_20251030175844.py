...
def crear_reserva_api(request):
    """API para crear reservas REALES en la base de datos - VERSIÓN CORREGIDA"""
    if request.method == 'POST':
        try:
            ...
            try:
                NotificacionService.notificar_creacion_reserva(reserva)
    ...
    if request.method == 'POST':
        try:
            ...
            if reserva.estado in ['Pendiente', 'Aprobada']:
                ...
                # NOTIFICAR AL USUARIO - NUEVO CÓDIGO
...
def aprobar_reserva_api(request, reserva_id):
    """API para que administradores aprueben reservas"""
    if request.method == 'POST':
        try:
            ...
            reserva.comentario_admin = request.POST.get('comentario', '')
            reserva.save()
            
            # NOTIFICAR AL USUARIO
            NotificacionService.notificar_aprobacion_reserva(
                reserva, 
            ...
            HistorialAprobacion.objects.create(
                ...
                motivo=reserva.comentario_admin
            )
            
            return JsonResponse({'success': True, 'message': 'Reserva aprobada exitosamente'})
...
def rechazar_reserva_api(request, reserva_id):
    """API para que administradores rechacen reservas"""
    if request.method == 'POST':
        try:
            # Verificar que el usuario es administrador
            perfil = PerfilUsuario.objects.get(user=request.user)
            ...
            reserva.id_aprobador = request.user
            reserva.comentario_admin = request.POST.get('motivo', '')
            reserva.save()
            
            # NOTIFICAR AL USUARIO
            NotificacionService.notificar_rechazo_reserva(
                reserva, 
                reserva.comentario_admin
            )
            ...
            return JsonResponse({'success': True, 'message': 'Reserva rechazada exitosamente'})
