...
def usuario_required(view_func):
    """Decorator para vistas solo de usuarios normales"""
    def _wrapped_view(request, *args, **kwargs):
        if not request.user.is_authenticated:
            return redirect('login_usuario')
        if hasattr(request.user, 'perfilusuario') and request.user.perfilusuario.rol == 'Usuario':
            return view_func(request, *args, **kwargs)
        else:
            messages.error(request, 'Acceso denegado. Esta área es solo para usuarios.')
            return redirect('login_usuario')
    return _wrapped_view

def admin_required(view_func):
    """Decorator para vistas solo de administradores"""
    def _wrapped_view(request, *args, **kwargs):
        if not request.user.is_authenticated:
            return redirect('login_admin')
        if request.user.is_superuser or (hasattr(request.user, 'perfilusuario') and request.user.perfilusuario.rol == 'Admin'):
            return view_func(request, *args, **kwargs)
        else:
...
def crear_reserva_view(request):
    if request.method == 'POST':
        try:
            ...
            messages.error(request, 'Error al crear la reserva. Verifica los datos.')
...
def revisar_solicitud_view(request, reserva_id):
    ...
    if request.method == 'POST':
        ...
        Notificacion.objects.create(
            ...
            titulo=f'Reserva {accion.lower()}',
            mensaje=f'Tu reserva para {reserva.espacio.nombre} ha sido {accion.lower()}. Motivo: {motivo}'
        )
        
        messages.success(request, f'Solicitud {accion.lower()} exitosamente.')
