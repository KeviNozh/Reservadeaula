...
# OPCIÓN 1: FORMULARIOS CORRECTOS CON LOS CAMPOS QUE SÍ EXISTEN
# (Primero debes agregar los campos al modelo como te mostré)

class ElementoPrestamoForm(forms.ModelForm):
    class Meta:
        model = ElementoReserva
        fields = ['fecha_prestamo', 'prestado']  # Solo campos que existen actualmente
        widgets = {
...
class ElementoDevolucionForm(forms.ModelForm):
    class Meta:
        model = ElementoReserva
        fields = ['fecha_devolucion', 'devuelto']  # Solo campos que existen actualmente
        widgets = {
...
# OPCIÓN 2: FORMULARIOS CON CAMPOS PERSONALIZADOS (sin modificar el modelo)
# Si prefieres NO modificar el modelo, usa estos formularios:

"""
class ElementoPrestamoForm(forms.ModelForm):
    observaciones_prestamo = forms.CharField(
        required=False,
        widget=forms.Textarea(attrs={
            'rows': 2,
            'class': 'form-control',
            'placeholder': 'Observaciones al momento del préstamo...'
        }),
        label='Observaciones'
    )
