...
def calendario_view(request):
    ...
    try:
        ...
        print(f"Error en calendario_view: {e}")
        return render(request, 'reservas/calendario.html', {'eventos': '[]', 'reservas': []})
    ...
    if request.method == 'POST':
        try:
            ...
            return JsonResponse({
                ...
                'redirect_url': '/reserva-exitosa/'  # ← AÑADE ESTA LÍNEA
            })
...
def reserva_exitosa_view(request):
    ...
    try:
        if reserva_id:
            ...
            reserva = Reserva.objects.filter(solicitante=request.user).order_by('-fecha_solicitud').first()
