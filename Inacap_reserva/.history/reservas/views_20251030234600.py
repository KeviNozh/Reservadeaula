...
def crear_espacio_api(request):
    """API PARA CREAR ESPACIO - VERSIÓN DEFINITIVA"""
    print("=" * 50)
    ...
    try:
        ...
        try:
            ...
            return JsonResponse({'success': False, 'error': 'Error en formato JSON: ' + str(e)})
        ...
        # Procesar piso (campo opcional)
        piso = None
        piso_str = data.get('piso')
        if piso_str is not None and str(piso_str).strip():
            try:
                piso = int(piso_str)
                if piso < 0:
        ...
        try:
            ...
            return JsonResponse({'success': False, 'error': 'Error al guardar en la base de datos: ' + str(e)})
        
    except Exception as e:
        print("💥 ERROR NO CONTROLADO:", str(e))
        import traceback
        traceback.print_exc()
        return JsonResponse({'success': False, 'error': 'Error interno del servidor: ' + str(e)})
