...
def crear_espacio_api(request):
    """API PARA CREAR ESPACIO - VERSIÓN SIMPLIFICADA PARA DEBUGGING"""
    print("=" * 50)
    print("🔍 INICIANDO crear_espacio_api - VERSIÓN DEBUG")
    ...
    try:
        ...
        if not data.get('tipo'):
            return JsonResponse({'success': False, 'error': 'Tipo es requerido'})
        
        if not data.get('capacidad'):
            return JsonResponse({'success': False, 'error': 'Capacidad es requerida'})
        
        # Crear espacio con datos mínimos
        espacio = Espacio(
            nombre=data['nombre'],
            tipo=data['tipo'],
        ...
        espacio.save()
        print(f"✅ ESPACIO CREADO: {espacio.id}")
        ...
        import traceback
        traceback.print_exc()
        return JsonResponse({'success': False, 'error': f'Error general: {str(e)}'})
