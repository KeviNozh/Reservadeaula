...
class ThemeManager {
    ...
    handleToggleClick() {
        // animación suave y luego forzar recarga corta para garantizar aplicación completa
        try{
            document.documentElement.style.transition = 'opacity 160ms ease';
            document.documentElement.style.opacity = '0.0';
        }catch(e){}
        // aplicar cambio de tema inmediatamente (persistido en localStorage)
        this.toggleTheme();
        // recargar para que todas las plantillas y assets se apliquen sin estado inconsistente
        setTimeout(() => { try{ location.reload(); }catch(e){} }, 220);
    }
